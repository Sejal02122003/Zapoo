...
export const listApprovedRestaurants = async (query = {}) => {
    ...
    // Enforce strict zone filtering (with auto-detection from lat/lng if zoneId is omitted)
    let targetZoneId = String(query.zoneId || '').trim();
    ...
    } else if (query.zoneId || (lat !== null && lng !== null)) {
        // Zone explicitly passed but not found, or coordinates fall outside all active zones -> return empty list
...
export const listPublicOffers = async () => {
    ...
    const mappedFoodOffers = list.map((o) => {
        ...
        if (o.discountType === 'percentage') {
            title = `${Number(o.discountValue) || 0}% OFF`;
            if (o.maxDiscount) title += ` up to ₹${o.maxDiscount}`;
        } else {
            title = `₹${Number(o.discountValue) || 0} OFF`;
            if (o.minOrderValue) title += ` above ₹${o.minOrderValue}`;
        }
    ...
    const bestOffers = allOffers.slice(2);

    // If there are less than 2 offers, just use all for both to show something in UI
    const finalDailyDeals = dailyDeals.length > 0 ? dailyDeals : allOffers.map(o => ({ ...o, dealText: o.title }));
    const finalBestOffers = bestOffers.length > 0 ? bestOffers : allOffers;

    return { allOffers, dailyDeals: finalDailyDeals, bestOffers: finalBestOffers, groupedByOffer: {} };
};
