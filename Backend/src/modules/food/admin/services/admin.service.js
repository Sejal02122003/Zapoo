...
export async function getTransactionReport(query = {}) {
    ...
    const transactions = transactionRows.map((tx) => {
        ...
        return {
            ...
            couponDiscount: Number(pricing.couponDiscount ?? 0) + Number(pricing.restaurantCouponDiscount ?? 0),
            cashbackAmount: Number(pricing.cashbackAmount || tx.amounts?.cashbackAmount || order.cashbackAmount || 0),
