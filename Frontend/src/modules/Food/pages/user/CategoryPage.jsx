...
export default function CategoryPage() {
  ...
  useEffect(() => {
    if (cart && cart.items) {
      ...
      cart.items.forEach(item => {
        newQuantities[`${item.restaurantId}-${item.itemId}`] = item.quantity
      })
  ...
  const handleItemClick = (item, restaurant) => {
    addToCart({
      ...
      image: item.image,
      isVeg: item.foodType === 'veg' || item.isVeg,
    ...
    <div className={`min-h-screen bg-white dark:bg-[#0a0a0a] ${shouldShowGrayscale ? 'grayscale opacity-75' : ''}`}>
      ...
      {selectedCategory !== 'all' && (
        <div className="relative w-full overflow-hidden bg-gradient-to-b from-primary via-orange-400 to-white dark:to-[#0a0a0a] pt-4 sm:pt-8 pb-10 sm:pb-12">
          {(() => {
            const categoryName = categories.find(c => (c.slug === selectedCategory) || (c.id === selectedCategory))?.name || selectedCategory;
      ...
      <div className="px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 py-4 sm:py-6 md:py-8 lg:py-10 space-y-6 md:space-y-8 lg:space-y-10">
        <div className="max-w-7xl mx-auto">
          ...
          <section className="relative mt-8 md:mt-12">
            ...
            {/* Items Grid */}
            <div className={`transition-opacity duration-300 ${showRestaurantSkeleton ? 'opacity-50' : 'opacity-100'}`}>
              {(() => {
                const allCategoryItems = filteredAllRestaurants.flatMap(restaurant => {
                  ...
                  return dishes.map(item => ({
                    ...
                    restaurantSlug: restaurant.slug || (restaurant.name || restaurant.restaurantName || '').toLowerCase().replace(/\s+/g, "-"),
                    isClosed: restaurant.isAcceptingOrders === false,
                    restaurantRaw: restaurant
                  }))
                });

                if (allCategoryItems.length === 0 && !showRestaurantSkeleton) {
                  return (
                ...
                return (
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 lg:gap-6 pb-12">
                    {allCategoryItems.slice(0, visibleCount * 5).map((item, index) => {
                      const quantity = quantities[`${item.restaurantId}-${item.itemId || item.id || item._id}`] || 0;
                      return (
                        <div key={`${item.restaurantId}-${item.itemId || item.id || item._id}-${index}`} className={`flex flex-col rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-800 shadow-sm bg-white dark:bg-[#1a1a1a] transition-transform hover:-translate-y-1 hover:shadow-md ${item.isClosed ? 'opacity-70 grayscale' : ''}`}>
                          {/* Item Image */}
